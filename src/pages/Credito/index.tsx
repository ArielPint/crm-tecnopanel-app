...
export default function Credito() {
  ...
  useEffect(() => {
    supabase
      ...
      .eq('etapa_actual', 'Evaluación Crediticia')
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-6 py-4 border-b border-gray-200 bg-white">
        <h1 className="text-lg font-bold text-gray-800">Eval. Crediticia</h1>
        <p className="text-xs text-gray-500">{opps.length} oportunidades</p>
      </div>
      <div className="flex-1 overflow-auto p-6">
        {opps.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-400">
            <p className="text-sm">Sin oportunidades en Eval. Crediticia</p>
          </div>
        ) : (
              ...
              <div key={o.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-red-200 transition-all">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right flex-shrink-0">
                    {o.monto_estimado && <p className="text-sm font-bold" style={{ color: '#ed3224' }}>$ {o.monto_estimado.toLocaleString('es-CL')}</p>}
